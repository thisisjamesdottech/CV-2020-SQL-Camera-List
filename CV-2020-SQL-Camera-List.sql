...
Purpose:    Produces a report of all cameras in the CompleteView database and their associated make, model,
...
*/
SELECT a.CameraName AS 'Camera Name',v.Manufacturer AS 'Make',v.Model AS 'Model',v.ChannelCount AS '# of Channels',c.ChannelId AS 'Channel ID',v.IpAddressV4 AS 'IP Address',v.MacAddress AS 'MAC Address'
FROM CompleteView.dbo.VideoDevices v
INNER JOIN CompleteView.dbo.Device d
ON v.DeviceRelationId=d.DeviceRelationId
INNER JOIN CompleteView.dbo.CameraVideoDevice c
ON d.DeviceId=c.DeviceId
INNER JOIN CompleteView.dbo.Camera a
ON c.CameraRelationId=a.CameraRelationId
-- Below IsDeleted = 0 will remove any cameras that have been deleted from the database
WHERE c.IsDeleted = 0